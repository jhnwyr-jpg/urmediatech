...
function register(){
  ...
  .then(function(r){return r.json()}).then(function(d){if(d.api_key){ss(d.api_key);apiKey=d.api_key;checkStatus()}}).catch(function(){});
}
function checkStatus(){
  fetch(E+'?action=status',{headers:{'x-api-key':gs()}})
  .then(function(r){return r.json()})
  .then(function(d){
    if(d.features){
      var enabled={};
      d.features.forEach(function(f){enabled[f.feature_key]=f.is_enabled});
      if(enabled.broadcast)loadBroadcastWidget();
    }
  }).catch(function(){})
}
...
function loadBroadcastWidget(){
  ...
  function mar(id){var ids=gri();if(ids.indexOf(id)===-1)ids.push(id);if(ids.length>200)ids=ids.slice(-200);localStorage.setItem(RK,JSON.stringify(ids))}
  function marAll(){var ids=notifications.map(function(n){return n.id});localStorage.setItem(RK,JSON.stringify(ids))}
  function isR(id){return gri().indexOf(id)!==-1}
  function gti(){try{return JSON.parse(localStorage.getItem(TK)||'[]')}catch(e){return[]}}
  function mt(id){var ids=gti();if(ids.indexOf(id)===-1)ids.push(id);if(ids.length>200)ids=ids.slice(-200);localStorage.setItem(TK,JSON.stringify(ids))}
  function wt(id){return gti().indexOf(id)!==-1}
  ...
  var pnl=document.createElement('div');pnl.className='urb-panel';pnl.innerHTML='<div class="urb-panel-header"><h3>\\ud83d\\udce2 Updates</h3><button id="urb-mark-all">Mark all read</button></div><div class="urb-panel-body" id="urb-list"></div>';document.body.appendChild(pnl);

  btn.addEventListener('click',function(){isOpen=!isOpen;pnl.classList.toggle('open',isOpen);if(isOpen)renderN()});
  document.getElementById('urb-mark-all').addEventListener('click',function(){marAll();updateB();renderN()});

  var fetching=false;
  function fetchN(){
    if(fetching)return;fetching=true;
    fetch(E+'?action=broadcast_list').then(function(r){return r.json()}).then(function(data){
      notifications=data.notifications||[];updateB();if(isOpen)renderN();
      for(var i=0;i<notifications.length;i++){var n=notifications[i];if(!isR(n.id)&&!wt(n.id)){mt(n.id);playSound();showToast(n);break}}
    }).catch(function(){}).finally(function(){fetching=false});
  }
  function getUC(){return notifications.filter(function(n){return!isR(n.id)}).length}
  function updateB(){var c=getUC();bdg.textContent=c>9?'9+':c;bdg.style.display=c>0?'flex':'none'}
  function escH(t){var d=document.createElement('div');d.appendChild(document.createTextNode(t));return d.innerHTML}
  ...
  function renderN(){
    ...
    list.innerHTML=notifications.map(function(n){
      var date=new Date(n.created_at).toLocaleDateString();var unread=!isR(n.id);
      var img=n.image_url?'<img class="urb-notif-img" src="'+escA(n.image_url)+'" alt="" onerror="this.style.display=\\'none\\'">':'';
      var rawUrl=n.url&&n.url.trim()?n.url.trim():'';if(rawUrl&&!/^https?:\\/\\//i.test(rawUrl))rawUrl='https://'+rawUrl;
      var link=rawUrl?'<a class="urb-notif-link" href="'+escA(rawUrl)+'" target="_blank" rel="noopener">\\ud83d\\udd17 Details</a>':'';
      return'<div class="urb-notif '+(unread?'unread':'')+'" data-id="'+n.id+'"><p class="urb-notif-title">'+escH(n.title)+'</p><p class="urb-notif-msg">'+escH(n.message)+'</p><span class="urb-notif-time">'+date+'</span><div class="urb-notif-detail" data-detail="'+n.id+'"><p class="urb-notif-detail-msg">'+escH(n.message)+'</p>'+img+link+'</div></div>';
    }).join('');
    list.querySelectorAll('.urb-notif').forEach(function(el){el.addEventListener('click',function(e){if(e.target.tagName==='A')return;var id=el.getAttribute('data-id');mar(id);el.classList.remove('unread');updateB();var det=el.querySelector('.urb-notif-detail');if(det)det.classList.toggle('open')})});
  }
  function showToast(notif){
    var t=document.createElement('div');t.style.cssText='position:fixed;top:24px;right:24px;z-index:100000;background:#fff;border-radius:12px;padding:16px;box-shadow:0 8px 30px rgba(0,0,0,0.12);max-width:340px;border-left:4px solid #7B5FFF;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;cursor:pointer;animation:urb-slide 0.3s ease';
    t.innerHTML='<p style="margin:0 0 4px;font-weight:600;font-size:14px;color:#1a1a1a">'+escH(notif.title)+'</p><p style="margin:0;font-size:13px;color:#666">'+escH(notif.message)+'</p>';
    if(notif.url)t.addEventListener('click',function(){window.open(notif.url,'_blank')});document.body.appendChild(t);
    var a=document.createElement('style');a.textContent='@keyframes urb-slide{from{transform:translateX(100%);opacity:0}to{transform:translateX(0);opacity:1}}';document.head.appendChild(a);
    setTimeout(function(){t.style.transition='opacity 0.3s';t.style.opacity='0';setTimeout(function(){t.remove()},300)},5000);
  }
  fetchN();window.__URB_INTERVAL=setInterval(fetchN,POLL);
